    ...
    $username=$user.SamAccountName
        if($CheckFolderSize -eq "yes"){
            if(test-path $UserDataPath\$username){
                $usersize = (Get-ChildItem $UserDataPath\$username -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum -ErrorAction SilentlyContinue).Sum / 1MB
...
$username=$user.SamAccountName
Move-Item $UserDataPath\$username $ArchiveDataPath\$username -ErrorAction SilentlyContinue
}
}
